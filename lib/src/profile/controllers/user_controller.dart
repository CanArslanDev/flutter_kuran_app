...
class UserControllerImpl extends UserController {
  ...
  var fileImage = ui.File("").obs;

  Future<bool> createAvatar() async {
    var baseUrl = "https://readyplayer.me/avatar";
    final resp = await url.launchUrl(Uri.parse(baseUrl));
    return resp;
  }
  ...
  void pickImage() async {
    List<Media>? res = await ImagesPicker.pick(
        count: 1, pickType: PickType.image, language: Language.English);

    if (res != null) {
      for (var item in res) {
        fileImage.value = ui.File(item.path);
        log(item.path);
      }
    }
  }
  ...
  pickerFile() async {
    ...
    if (result != null) {
      result.paths;
      fileImage.value = ui.File(result.files.first.path.toString());
    } else {
  ...
  Future<void> pickImageFromGalery() async {
    try {
      ...
      if (file != null) {
        fileImage.value = ui.File(file.path);
      }
