...
class HistoryQuizHomePage extends StatefulWidget {
  const HistoryQuizHomePage({Key? key}) : super(key: key);

  @override
  State<HistoryQuizHomePage> createState() => _HistoryQuizHomePageState();
}

class _HistoryQuizHomePageState extends State<HistoryQuizHomePage> {
  List gameList = [];
  List gameImg = [];
  List dataNormal = [];
  List dataHard = [];
  ...
  Future start() async {
    final String responseNormal = await rootBundle.loadString(
        'assets/quiz/history_quiz_normal_' + 'langCode'.tr() + '.json');
    final List decodedDataNormal = json.decode(responseNormal);
    final String responseHard = await rootBundle.loadString(
        'assets/quiz/history_quiz_hard_' + 'langCode'.tr() + '.json');
  ...
  Widget games(BuildContext context) {
    return Padding(
      ...
      child: InkWell(
        onTap: () {
          Get.to(QuizPage(datas: dataNormal), curve: Curves.easeOut)
              ?.then((value) {
            start();
