...
class PrayerTimeNotifController extends GetxController {
  ...
  void createPrayerTimeNotif(String prayerTime) {
    ...
    AwesomeNotify.createBasicNotif(
      id: uniqueId,
      title: "ezanvakti".tr(),
      body: "${prayerTime.capitalize} " + "namazvakitgeldi".tr(),
    ).then((value) {
      if (!value) {
        Get.snackbar("hayaksi".tr(), "bildirimeizin".tr());
      }
    });
  }
  ...
  void createPrayerTimeReminder({
    ...
    AwesomeNotify.createScheduleNotif(
      id: uniqueId,
      title: "ezanvaktihatirlatici".tr(),
      body: minute != 0
          ? "${prayerTime.capitalize} " +
              "namazvakityaklasiyor".tr() +
              " ${Duration(seconds: minute).inMinutes}, " +
              "hazirlanalim".tr()
          : "${prayerTime.capitalize} " + "ibadetet".tr(),
      dateTime: dateTime,
      hour: hour,
      minute: minute,
    ).then((bool value) {
      if (!value) {
        Get.bottomSheet(
          AppPermissionStatus(
            ...
            title: "bildirimizin".tr(),
            message: "bildirimgonder".tr(),
            onPressed: () {
              AwesomeNotifications()
                ...
                (isAllowed) {
                  if (isAllowed == true) {
                    ...
                    Get.snackbar("hayaksi".tr(), "bildirimizinverilmedi".tr());
                  }
                },
              );
            },
